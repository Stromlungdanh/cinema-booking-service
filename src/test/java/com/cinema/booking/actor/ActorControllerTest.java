...
class ActorControllerTest {
    ...
    private ActorResponse sampleResponse(Long id) {
        return new ActorResponse(id, "Leonardo DiCaprio", "http://avatar.jpg");
    }

    @Test
    void create_returns201WithBody() throws Exception {
        when(actorService.create(any())).thenReturn(sampleResponse(1L));
    ...
    void create_returns400WhenNameBlank() throws Exception {
        ActorRequest invalid = new ActorRequest("", "http://avatar.jpg");
    ...
    void findById_returns404WhenMissing() throws Exception {
        when(actorService.findById(eq(99L)))
                .thenThrow(new ResourceNotFoundException("Khong tim thay dien vien voi id=99"));

        mockMvc.perform(get("/api/admin/actors/99"))
                .andExpect(status().isNotFound())
                .andExpect(jsonPath("$.message").value("Khong tim thay dien vien voi id=99"));
    }

    @Test
    void delete_returns204() throws Exception {
        mockMvc.perform(delete("/api/admin/actors/1"))
